...
const tagType = ETagType.RectangularBracelet;
...
const stringContainer2 = ref<HTMLDivElement | null>(null);
...
const isMobile = ref<number | null>(null);
...
function getBackgoundNthColor(
  ...
  return (
    field
      ...
      ?.children?.filter((item) => item.type == "color-grid")
    [position]?.config.options.find((item: any) => item.selected)?.value ?? defaultColor
  );
}
...
<template>
  <div class="tag-item" ref="tagItem">
    <div :style="{
      ...
      <div :style="{
        background: getBackgoundNthColor(
          props.field,
          1,
          `${getBackgroundColor(props.field)}40`
        ),
        width: `${!isMobile ? `calc(${width * scaleFactor}cm - ${2 * scaleFactor}cm)` : `${isMobile - 100}px`}`,
        height: `${height * scaleFactor}cm`,
        paddingTop: `${0.1 * scaleFactor}cm`,
        marginBottom: '5px',
        paddingBottom: `${0.1 * scaleFactor}cm`,
        paddingLeft: `${2.5 * scaleFactor}cm`,
      }">
        <div class="tag" ref="stringContainer" :style="{
          display: 'flex',
          fontFamily: getTextFont(props.field).familyName,
          background: getBackgound(props.field),
          marginLeft: hasImage ? `${3 * scaleFactor}rem` : 0,
          width: `${8 * scaleFactor}cm`,
          position: 'relative',
          height: `100%`,
          ...
          <div :style="{
            width: `calc(100% - ${hasImage ? `${1.5 * scaleFactor}cm` : '0px'})`,
            display: 'grid',
            gridAutoFlow: 'row',
          }" ref="stringContainer2">
            <div ref="container_stringSpan_1" v-if="getText(props.field, 0, [EItemType.text])" :style="{
              width: '100%',
              overflow: 'hidden',
            }">
              <span class="tag-text" ref="stringSpan_1" :style="{
            ...
            <div ref="container_stringSpan_2" v-if="getText(props.field, 1, [EItemType.text])" :style="{
              ...
              <span class="tag-text" ref="stringSpan_2" :style="{
                color: getTextColor(props.field),
                textShadow: `${textShadow(
                  builder,
                  getTextColor(props.field),
            ...
            <div ref="container_stringSpan_3" v-if="getText(props.field, 2, [EItemType.text])" :style="{
              width: '100%',
              overflow: 'hidden',
            }">
              <span class="tag-text" ref="stringSpan_3" :style="{
                color: getTextColor(props.field),
                textShadow: `${textShadow(
                  builder,
                  getTextColor(props.field),
      ...
      <div v-for="index in 8" :key="index" :style="{
        background: getBackgoundNthColor(
          props.field,
          1,
          `${getBackgroundColor(props.field)}40`
        ),
        width: `${0.2 * scaleFactor}cm`,
        height: `${height * scaleFactor}cm`,
        marginLeft: `${2 * scaleFactor}px`,
      }" />
    </div>
    <div class="tag-description" :style="{ marginLeft: `${25 * scaleFactor}px`, }">
      <p class="tag-title m-0">Pulseira</p>
