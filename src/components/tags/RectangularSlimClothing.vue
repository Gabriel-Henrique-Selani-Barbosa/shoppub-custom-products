...
const width = props.width || 3.9;
const height = props.height || 0.6;
const tagType = ETagType.RectangularSlimClothing;
...
function getBackgound(field: IStep<any, any, any>[]) {
  let response = base64Image.value;
  if (builder.steps
...
watch(
  () => getSpans().map((item) => getText(props.field, item.indexState, [EItemType.textSmall])),
...
<template>
  <div class="tag-item" ref="tagItem">
    <div class="clothing-tag" ref="stringContainer" :style="{
      marginTop: '20px',
      marginLeft: '25px',
      position: 'relative',
      ...
      <div ref="container_stringSpan_1" :style="{
        ...
        <span class="clothing-tag-text string-container" ref="stringSpan_1" :style="{
          maxWidth: `${width * scaleFactor}cm`,
          ...
          {{ getText(props.field, 0, [EItemType.textSmall]) }}
        </span>
      </div>
    </div>
    <div class="tag-description" :style="{ marginLeft: '25px' }">
      <p class="tag-title m-0">Etiqueta Fina Para Roupas</p>
