...
const tagType = ETagType.SquareGift;
...
const updateValue = (novoValor: unknown[], valorAnterior: unknown[]): void => {
  ...
  if (!container) return;
  resizeFont(indexState, 1, 20 * scaleFactor.value);
};
watch(
  () => getSpans().map((item) => getText(props.field, item.indexState, [EItemType.text])),
  ...
  <div class="tag-item" ref="tagItem">
    <div
      :style="{
        ...
        height: `${height * 1.1 * scaleFactor}cm`,
      }"
    >
      ...
      >
        <div
          ...
          :style="{
            width: '100%',
            overflow: 'hidden',
            minWidth: `calc(${width * scaleFactor}cm - 0.5rem)`,
          }"
        >
          <span
      ...
      >
        <div :style="{alignSelf: 'start', marginTop: `${6 * scaleFactor}px`}">
          <span
            class="medium-tag-text string-container"
            ref="stringSpan_3"
            :style="{
    ...
    <div class="tag-description" :style="{ marginLeft: '25px' }">
      <p class="tag-title m-0">Etiqueta Quadrada De Alerta</p>
