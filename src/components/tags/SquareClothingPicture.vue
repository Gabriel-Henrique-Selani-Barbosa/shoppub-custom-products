...
const tagType = ETagType.SquareClothingPicture;
const scaleFactorDefault = 1.3
...
const updateValue = (novoValor: unknown[], valorAnterior: unknown[]): void => {
  ...
  if (!valorAnterior[indexState]) {
    novoValor.forEach((_, index) => {
      resizeFont(index, textCount, container.offsetHeight);
    });
  } else {
    resizeFont(indexState, textCount, container.offsetHeight);
  }
};
watch(
  () => getSpans().map((item) => getText(props.field, item.indexState, [EItemType.text])),
...
<template>
  <div class="tag-item" ref="tagItem">
    <div class="square-group" :style="{ marginTop: '20px', marginLeft: '25px', position: 'relative' }">
          ...
          <div ref="container_stringSpan_1" v-if="getText(props.field, 0, [EItemType.text])" :style="{
            ...
            <span class="square-tag-text string-container" ref="stringSpan_1" :style="{
              ...
              textShadow: `${textShadow(
                ...
                hasImage ? '#ffffff' : getBackgroundColor(props.field),
                base64ColorPalette,
                hasImage,
    ...
    <div class="tag-description" :style="{ marginLeft: '25px' }">
      <p class="tag-title m-0">Etiqueta Quadrada Para <br />Roupas Com Foto</p>
