...
if (props.field.config && !ensure(props.field.config.options).length) {
...
watch(() => props.field.config && ensure(props.field.config.options)[0]?.value, () => {
  props.field.config.checked = props.field.config.required ? Boolean(ensure(props.field.config.options)[0].value) : true;
})

onMounted(() => {
  if (props.field.config) props.field.config.checked = props.field.config?.required ? Boolean(ensure(props.field.config.options)[0].value) : true;
})
</script>

<template>
  <FormGroup :required="Boolean(field.config?.required)" :style="{ marginBottom: '.5rem' }">
    <template v-if="$slots.prepend" #prepend>
      <slot name="prepend" />
    </template>
    <input
      :placeholder="`${field.label} ${Boolean(field.config?.required) ? '*' : ''}`"
      v-if="props.field.config && ensure(field.config.options)[0]"
      type="text"
      maxlength="40"
      :class="{ featured: field.config?.featured || true }"
      v-model.lazy="ensure(field.config.options)[0].value"
      v-debounce="50"
    />
